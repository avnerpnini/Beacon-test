...
function loadXMLDoc(action) {
    ...
    if (action=="addUser"){
        if(userID>0){
            ...
            xmlhttp.send("navID="+navID+"&routeNum="+routeNum+"&phoneN="+phoneN+"&firstN="+firstN+"&lastN="+lastN+"&city="+city+"&mail="+mail+"&agree="+agree+"&phoneT=model:"+Dmodel+"&browser="+Dcordova+"&version="+Dversion+"&OS="+Dplatform+"&IP="+Duuid+"&password="+password);
        }
    }
    else
        xmlhttp.send();
...
function stoargeUserID(){
    ...
    var d1 = new Date(); 
    var theTime1 = d1.getTime()/1000;
    localStorage.setItem("userID", userID);
    localStorage.setItem("startTime", theTime1);
...
function isStoargeUserID(){
    if (localStorage.getItem("userID") > 0){
        ...
        if (startTime + (3600 * 24) > nowTime){
            userID = localStorage.getItem("userID");
            location.assign("index.html#nav");
            loadXMLDoc("getQuestion");
            $("#liUserId").html("מספר משתמש: "+userID);
            localStorage.setItem("oldVersion", "true");
        }
    }
...
function backToNewVersion(){
    if (typeof(StatusBar) !=  "undefined" )
        StatusBar.hide();
    localStorage.setItem("oldVersion", "false");
    localStorage.setItem("backToNewVersion", 1);
